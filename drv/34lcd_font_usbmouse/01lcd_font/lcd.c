 ...
 * @info: 目标window的对象
 * @ch: 目标字符的asc码
 * @color: 用于画字符的颜色，RGB888格式
 * @font: 字库对象
 * @x, y: 在屏幕上画出目标字符的左上角坐标
 */
static void __draw_ch(struct win_info *info, char ch, u32 color, struct font_desc *font_info, size_t x, size_t y)
{
	//字库中字符每一个行占据多少字节
	size_t line_length = (font_info->width + 7) / 8;
	//字库中单个字符占据多少字节
	size_t font_size = font_info->height * line_length;

	//指向目标字符对应的字库数据的首地址的指针
	char (*font)[line_length] = (char (*)[line_length])((u8*)font_info->data + ch * font_size);

	//遍历单个字库文件像素坐标的循环变量
	size_t ix, iy;
	for (iy = 0; iy < font_info->height; ++iy) {
		for (ix = 0; ix < font_info->width; ++ix) {
			if ((*font)[iy] & (1 << (font_info->width - ix - 1))) {
...
void draw_ch(u8 ch)
{
	//字符光标的坐标
	static u32 x, y;
	struct font_desc *font = (struct font_desc *)&font_vga_8x16;

	if ((ch == '\n') || (ch == '\r')) {
		goto new_line;
	} else {
		__draw_ch(win, ch, RGB888(255, 255, 255), font, x, y);
		x += font->width;
		if (x + font->width >= XSIZE - 1) {
			goto new_line;
		}
	}
...
new_line:
	x = 0;
	y += font->height;
	//到了页末还换行时，强行限制始终回到页末最后一行
	if (y + font->height >= YSIZE - 1) {
		y -= font->height;
	}
}

...
struct ldm_info {
	...
	struct i2c_client *client; //指向本对象所在的i2c_client对象地址
...
static irqreturn_t ldm_handler(int irqno, void *data)
{
	...
	struct ldm_info *ldm = i2c_get_clientdata((struct i2c_client *)data);
...
static int ldm_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	input_set_abs_params(ldm->dev, ABS_X, 0, XSIZE-1, 0, 0);
	input_set_abs_params(ldm->dev, ABS_Y, 0, YSIZE-1, 0, 0);

	//5 硬件操作，申请中断
	ret = request_irq(IRQNO, ldm_handler, IRQF_TRIGGER_FALLING, DEVNAME, client);
	if (ret < 0) {

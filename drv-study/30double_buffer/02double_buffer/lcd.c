 ...
 */
static void nodoublebuffer_test(struct win_info *info, size_t xlen, size_t ylen, size_t xstep, size_t ystep, u32 front_color, u32 back_color)
{
	printk("%s\n", __FUNCTION__);
	...
	for (; ; x += ixstep, y += iystep) {
		...
		if ((x == 0) && (y <= 0)) {
			...
			iystep = 0;
			y = 0;
		} else if ((x + xlen >= XSIZE - 1) && (y == 0)) { //右上角
			ixstep = 0;
			iystep = ystep;
			x = XSIZE - 1 - xlen;
		} else if ((x + xlen == XSIZE - 1) && (y + ylen >= YSIZE - 1) ) { //右下角
			ixstep = -xstep;
			iystep = 0;
			y = YSIZE - 1 - ylen;
		} else if ((x <= 0) && (y + ylen == YSIZE - 1)) { //左下角
			ixstep = 0;
			iystep = -ystep;
			x = 0;
		}
...
/* 将目标window的显存首末地址切换到新的地址上
 * @info: 目标window对象
 * @new_addr: 新的显存首地址
 */
static void change_buff(struct win_info *info, u32 new_addr)
{
	//轮询检查当前时刻是否适合做显存切换
	//如果当前处于帧周期的像素数据传输时间(active)，则不能切换，必须避开像素数据传输的时间段，否则会造成画面撕裂，因为一帧数据的上下半部分将分别来自不同的显存
	//while (((VIDCON1 >> 13) & 3) == 0b10) {}
	while (((VIDCON1 >> 13) & 3) != 0b11) {}

	switch (info - win) {
	case 0:
		VIDW00ADD0B0 = new_addr;
		VIDW00ADD1B0 = new_addr + XSIZE * YSIZE * info->pixel_size;
		break;
	}
}
 ...
 * @back_color: 背景色，RGB888格式
 */
static void doublebuffer_test(struct win_info *info, size_t xlen, size_t ylen, size_t xstep, size_t ystep, u32 front_color, u32 back_color)
{
	printk("%s\n", __FUNCTION__);

	//前景，正显示在window屏幕上的显存首地址
	u32 front_buff = info->start_addr;
	//背景，隐藏的不显示的显存，用于存放下一帧画面的像素数据，渲染完下一帧画面之后，将切换到前景去显示。与front_buff交替着指向不同的显存地址
	u32 back_buff = info->start_addr + XSIZE * YSIZE * info->pixel_size;
	u32 tmp_buff = 0; //用于交换前景和背景显存地址的临时变量

	//清屏
	clear_screen(info, back_color); //清除当前显存
	//清除背景
	info->start_addr = back_buff;
...
static void lcd_init()
{
	...
	doublebuffer_test(win, 100, 100, 5, 5, RGB888(255, 255, 0), RGB888(0, 0, 255));
}
